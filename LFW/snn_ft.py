...
def main():
    ...
    fdir = 'result_LFW/'+str(args.arch)+'_'+str(args.bit)+'bit' + '_ft'
    ...
    if args.init:
        if os.path.isfile(args.init):
            ...
            exit()
    print('=> loading LFW data...')   # #CIFAR10 -> MNIST
    trainloader = DataLoader(TrainSet, batch_size=args.batch_size, shuffle=True, drop_last=True, num_workers=2)
    testloader = DataLoader(TestSet, batch_size= 100, shuffle=False, drop_last=True, num_workers=2)
