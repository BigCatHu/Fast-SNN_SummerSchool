...
from models import *
from lfwdataset import *

parser = argparse.ArgumentParser(description='PyTorch LFW Training')
parser.add_argument('--epochs', default=300, type=int, metavar='N', help='number of total epochs to run')   # epochs=300
parser.add_argument('-a', '--arch', metavar='ARCH', default='res20')  # arch='res20'    
parser.add_argument('--start-epoch', default=0, type=int, metavar='N', help='manual epoch number (useful on restarts)') # start_epoch=0
parser.add_argument('-b', '--batch-size', default=128, type=int, metavar='N', help='mini-batch size (default: 128),only used for train')  # batch_size=1
...
def main():
    ...
    model=None
    if use_gpu:
        float = True if args.bit == 32 else False #Ouroboros
        # float = True
        if args.arch == 'alex':
    ...
    if not os.path.exists('result_LFW'):
        os.makedirs('result_LFW')
    fdir = 'result_LFW/'+str(args.arch)+'_'+str(args.bit)+'bit'
    ...
    testloader = DataLoader(TestSet, batch_size= 100, shuffle=True, drop_last=True, num_workers=2)    
