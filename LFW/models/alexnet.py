...
class AlexNet(nn.Module):
    def __init__(self, num_classes: int = 29, float=False) -> None:
        ...
        if float:
            ...
            self.layer5 = Dummy(nn.Sequential(nn.Conv2d(384, 256, 3, stride=2, padding=1),
                                              nn.BatchNorm2d(256),
                                              nn.ReLU(inplace=True)))	
            self.layer6 = Dummy(nn.Sequential(nn.Linear(4096*4, 2048),
                                              nn.BatchNorm1d(2048),
                                              nn.ReLU(inplace=True)))     
            self.layer7 = Dummy(nn.Linear(2048, 29))
            ...
            self.layer6 = Dummy(nn.Sequential(QuantLinear(4096*4, 2048),
                                            #   QuantLinear(4*4*256, 2048),
                                              nn.BatchNorm1d(2048),
                                              QuantReLU(inplace=True)))
        
            self.layer7 = Dummy(last_fc(2048, 29))
            self.flat = Dummy(nn.Flatten(1))
...
class S_AlexNet(nn.Module):
    def __init__(self, num_classes: int = 29, T: int = 3) -> None:
        ...
        self.layer5 = Spiking(nn.Sequential(QuantConv2d(384, 256, 3, stride=2, padding=1),
                                            nn.BatchNorm2d(256),
                                            IF()), T)	
        self.layer6 = Spiking(nn.Sequential(QuantLinear(4096*4, 2048),
                                            nn.BatchNorm1d(2048),
                                            IF()), T)
        self.layer7 = last_Spiking(last_fc(2048, 29),T)
